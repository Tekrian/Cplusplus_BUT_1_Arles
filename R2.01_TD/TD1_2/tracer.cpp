...
class Traceur : public EZWindow {
 ...
 Polynome poly; // poly est un pointeur sur polynome
  ...
  for(int i=0;i<getWidth();i+=pixel_step)
   {
    ...
    double       y = poly(x); // On declenche la fonction functeur sur la coordonnee x et on stocke la valeur dans y.   3 modif
...
public:
 Traceur(int w,int h,const char *name,const Polynome& _p,double _xmin,double _xmax,double _ymin,double _ymax,unsigned short _pixel_step)
  : EZWindow(w,h,name),
    poly(_p),xmin(_xmin),xmax(_xmax),ymin(_ymin),ymax(_ymax),pixel_step(_pixel_step),   //2 modif
    curi(-1),curj(-1)
