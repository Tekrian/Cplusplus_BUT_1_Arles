...
// SVP Monsieur j'ai mis  "MODIFICATION" pour montrer là où j'ai modifier 
...
class Traceur : public EZWindow {
 ...
 Polynome poly; // poly est un pointeur sur polynome        MODIFICATION
  ...
  for(int i=0;i<getWidth();i+=pixel_step)
   {
    ...
    double       y = poly(x); // On declenche la fonction functeur sur la coordonnee x et on stocke la valeur dans y.       MODIFICATION 
 ...
 void trace_curseur()
 {
  ...
  ostringstream oss;
  if(curi>=0 && curi<int(getWidth()) && curj>=0 && curj<int(getHeight()) ) // Si le curseur est dans la fenetre,  1 Modif
   {
...
public:
 Traceur(int w,int h,const char *name,const Polynome& _p /*MODIFICATION*/,double _xmin,double _xmax,double _ymin,double _ymax,unsigned short _pixel_step)
  : EZWindow(w,h,name),
    poly(_p)/*MODIFICATION*/,xmin(_xmin),xmax(_xmax),ymin(_ymin),ymax(_ymax),pixel_step(_pixel_step),  
    curi(-1),curj(-1)
...
class MyApplication : public EZDraw {
  ...
  // On crée deux polynomes et deux traceurs pour les tracer dans deux fenêtres différentes
  double coef1[2] = {1, 2};
  double coef2[3] = {2, -5, 1};
  Polynome poly1;
  Polynome poly2;
 ...
 public:
  MyApplication()
   :
    ...
    tr1(640, 480, "fonction_1", poly1, -10.0, 10.0, -10.0, 10,3),
    tr2(640, 480, "fonction_2", poly2, -5.0, 5.0, -5.0, 20,3)
